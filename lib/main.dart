...
import 'package:apidemo/util/custom_interceptors.dart';
import 'package:dio/dio.dart';
import 'package:dio_flutter_transformer/dio_flutter_transformer.dart';
import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';

import 'bloc/availabejob/available_job_bloc.dart';
import 'network/retrofit_api_client.dart';
...
//
void main() {
  _setUpRetrofitClient();
//  _getAppToken();
  runApp(MyApp());
}

void _setUpRetrofitClient() {
  Dio dio = Dio();
  dio.options = getBaseOption();
//  dio.interceptors.add(CacheInterceptor());
  dio.interceptors.add(CustomInterceptors());
  dio.transformer = FlutterTransformer();
  Constant.client = RetrofitApiClient(dio);
}

BaseOptions getBaseOption() {
  if (Constant.applicationToken != null)
    return BaseOptions(
        connectTimeout: Constant.connectionTimeOut,
        receiveTimeout: Constant.connectionTimeOut,
        sendTimeout: Constant.connectionTimeOut,
        contentType: 'application/json',
        maxRedirects: 10,
        baseUrl: Constant.baseUrl,
        headers: {'Authorization': 'Bearer ' + Constant.applicationToken});
  else
    return BaseOptions(
        connectTimeout: Constant.connectionTimeOut,
        receiveTimeout: Constant.connectionTimeOut,
...
//  Constant.client
//      .getAvailableJob(AvailableJobRequest(
//          0,
//          0,
//          0,
//          0,
...
//          'string',
//          List<RequestId>(0),
//          HeaderRequest(1, 0, 1, 0, 1)))
//      .then((response) => setResponseValue(response))
//      .catchError((onError) => print(onError));
//}
//
//void setResponseValue(AvailableJobResponse response) {
//  Constant.applicationToken = response.data.jobEmployer.companyLogo;
//  print(Constant.applicationToken);
//}
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: BlocProvider(
        create: (context) => AvailableJobBloc(),
        child: MyHomePage(),
      ),
    );
  }
